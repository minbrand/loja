...
function pixDue_calculate(expira̤̣o){
    let momento = new Date;
	let expira = new Date(expira̤̣o);
	let diferen̤a = expira-momento;
	let minutos = Math.floor(diferen̤a/(1e3*60));
	let segundos = Math.floor(diferen̤a/1e3%60);
...
function pixDue_v2(){
    let now = new Date();
    let expira̤̣o = new Date(now.getTime() + 10*60000);
    let year = expira̤̣o.getFullYear();
    let month = expira̤̣o.getMonth()+1;
    let day = expira̤̣o.getDate();
    let hours = expira̤̣o.getHours();
    let minutes = expira̤̣o.getMinutes();
    let seconds = expira̤̣o.getSeconds();
	let m = 0;
	m = pixDue_rotate(m);
    expira̤̣o = year+'-'+month+'-'+day+' '+hours+':'+minutes+':'+seconds;
    pixDue = pixDue_calculate(expira̤̣o); 
    $('#pixDue').text(pixDue);
...
function checkoutExterno(acionamento){
	...
	if($('#situa̤̣oCheckoutExterno').text()=='ativo' && $('#acionamentoCheckoutExterno').text()==acionamento){
		let parametros = '';
		if($('#nomeCheckoutExterno').text()=='sim'){
			parametros = parametros+'nomeDoProduto='+$('#nomeDoProduto').text()+'&';
			}
		if($('#pre̤oOriginalCheckoutExterno').text()=='sim'){
			parametros = parametros+'pre̤oOriginalDoProduto='+$('#pre̤oOriginalDoProduto').text()+'&';
			}
		if($('#pre̤oAtualCheckoutExterno').text()=='sim'){
			parametros = parametros+'pre̤oAtualDoProduto='+$('#pre̤oAtualDoProduto').text()+'&';
			}
		...
		if($('#novaAbaCheckoutExterno').text()=='ṇo'){
...
function alterarVaria̤̣o(tipo,varia̤̣o,destino,fullid){
	$.ajax({
		url: $('#caminhoBase').text()+'/api/',type: "GET",async: true,data: "metodo=alterarVaria̤̣o&fullid="+fullid+"&tipo="+tipo+"&varia̤̣o="+varia̤̣o,dataType: "html",
		success: function(resposta){ resposta = resposta.trim();	
			if(destino.length>5){
				window.location.href = destino;
				}else{
					$('#'+tipo+'Atual').text(varia̤̣o);
					$('#'+tipo+'Varia̤̣o').text(varia̤̣o);
					for(c=0;c<10;c++){
						if(c==destino){
...
function buscar(busca,tipo){
	...
	if(tipo=='pesquisa'){
		...
		setcookie('appendPesquisa','ṇo');
		}
	$.ajax({
		url: $('#caminhoBase').text()+'/api/',type: "POST",async: true,data: 'metodo=configurarPagina̤̣o&tipoDeBusca='+tipo+'&busca='+busca,dataType: "html",
		success: function(resposta){ resposta = resposta.trim();	
...
function avalia̤̣o(id,fullid,tipo){
	...
	$.ajax({
		url: $('#caminhoBase').text()+'/api/',type: "POST",async: true,data: 'metodo=avalia̤̣o&id='+id+'&fullid='+fullid+'&tipo='+tipo+'&corLike='+corLike+'&corUnlike='+corUnlike+'&botaoLike='+botaoLike+'&botaoUnlike='+botaoUnlike+'&likes='+likes+'&unlikes='+unlikes,dataType: "html",
		success: function(resposta){ resposta = resposta.trim();	
...
function adicionarAoCarrinho(fullid){
	...
	for(c=0;c<totalDeVaria̵̤es;c++){
		if($('.tituloDaVaria̤̣o'+c).text()==' '){continue;}
		/*
		...
		*/
		tituloDaVaria̤̣o = $('.tituloDaVaria̤̣o'+c).text();
		atributoDaVaria̤̣o = $('.atributoDaVaria̤̣o'+c).text();
		varia̵̤es = varia̵̤es+tituloDaVaria̤̣o+':::'+atributoDaVaria̤̣o+'||';
		}

	$.ajax({
		url: $('#caminhoBase').text()+'/api/',type: "POST",async: true,data: "metodo=adicionarAoCarrinho&fullid="+fullid+'&cor='+cor+'&tamanho='+tamanho+'&voltagem='+voltagem+'&sabor='+sabor+'&quantidade='+quantidade+'&varia̵̤es='+varia̵̤es,dataType: "html",
...
function moverCarrossel(carrossel,dire̤̣o){
	if(dire̤̣o=='direita'){
